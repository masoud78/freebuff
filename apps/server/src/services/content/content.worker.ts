import type { ApiUsageStatus, GeminiUsage } from '@freebuff/contracts';
import { getDatabase } from '../../core/database/client.js';
import { apiUsage, type JobRow } from '../../core/database/schema.js';
import { DomainError } from '../errors.js';
import { GeminiGatewayError, geminiGateway, type GeminiGatewayLike } from '../gemini/gateway.js';
import { jobService, type DbExecutor } from '../jobs.service.js';
import { modelsService } from '../models.service.js';
import { settingsService } from '../settings.service.js';
import { batchService } from '../batches.service.js';
import { computeRetryDelayMs } from '../transcription/worker.js';
import { batchContentGenerationService } from './batch-content-generation.service.js';
import { batchFinalizationService } from '../knowledge/batch-finalization.service.js';

const POLL_INTERVAL_MS = 2000;

/**
 * Persistent content worker (Phase 11). Picks up CONTENT_GENERATION jobs of
 * started batches and generates content from the publishable delta only.
 * Fully idempotent via delta signature + (batch, destination, generation).
 */
export class ContentWorker {
  private activeCount = 0;
  private running = false;
  private timer: NodeJS.Timeout | null = null;

  constructor(private readonly gateway: GeminiGatewayLike = geminiGateway) {}

  get isRunning(): boolean {
    return this.running;
  }

  start(): void {
    if (this.running) return;
    this.running = true;
    this.timer = setInterval(() => void this.tick(), POLL_INTERVAL_MS);
    void this.tick();
  }

  stop(): void {
    this.running = false;
    if (this.timer) {
      clearInterval(this.timer);
      this.timer = null;
    }
  }

  wake(): void {
    void this.tick();
  }

  async processJob(job: JobRow): Promise<void> {
    try {
      await batchContentGenerationService.generateForJob(job, this.gateway);
      await batchFinalizationService.finalizeIfComplete(job.batchId);
    } catch (error) {
      await this.handleExecutionError(job, error);
    }
  }

  private async handleExecutionError(job: JobRow, error: unknown): Promise<void> {
    const { batchId } = job;
    const destinationId = job.entityId;

    if (error instanceof GeminiGatewayError) {
      const modelId = await modelsService.getConfiguredModelId('CONTENT_GENERATION').catch(() => null);
      await this.insertUsage(
        batchId,
        job.id,
        destinationId,
        modelId,
        null,
        error.durationMs,
        'FAILED',
        error.code,
      );
      await this.failJob(job, error.code, error.message, error.retryable, batchId, error);
      return;
    }
    if (error instanceof DomainError) {
      // Permanent configuration/validation failures — never loop forever.
      await this.failJob(job, error.code, error.message, false, batchId, error);
      return;
    }
    await this.failJob(
      job,
      'CONTENT_GENERATION_FAILED',
      'تولید محتوا با خطای غیرمنتظره مواجه شد.',
      true,
      batchId,
      error,
    );
  }

  private async failJob(
    job: JobRow,
    errorCode: string,
    errorMessage: string,
    retryable: boolean,
    batchId: number,
    cause?: unknown,
  ): Promise<void> {
    const nextAttemptAt = retryable ? new Date(Date.now() + computeRetryDelayMs(job.attempt)) : null;
    await jobService.markFailed(job.id, errorCode, errorMessage, { retryable, nextAttemptAt });
    console.error(`[content-worker] job ${job.id} failed`, {
      batchId,
      jobId: job.id,
      destinationId: job.entityId,
      operation: 'content_generation',
      errorCode,
      err: cause ?? errorCode,
    });
    await batchService.refreshBatchState(batchId);
  }

  private async insertUsage(
    batchId: number,
    jobId: number,
    destinationId: number,
    modelId: string | null,
    usage: GeminiUsage | null,
    durationMs: number | null,
    status: ApiUsageStatus,
    errorCode: string | null,
    db: DbExecutor = getDatabase(),
  ): Promise<void> {
    await db.insert(apiUsage).values({
      batchId,
      jobId,
      audioId: null,
      destinationId,
      stage: 'CONTENT',
      modelId,
      inputTokens: usage?.inputTokens ?? null,
      outputTokens: usage?.outputTokens ?? null,
      cachedTokens: usage?.cachedTokens ?? null,
      totalTokens: usage?.totalTokens ?? null,
      durationMs: durationMs ?? 0,
      status,
      errorCode,
      createdAt: new Date(),
    });
  }

  private async tick(): Promise<void> {
    if (!this.running) return;
    try {
      const concurrency = (await settingsService.getSettings()).processingConcurrency;
      while (this.activeCount < concurrency) {
        const job = await jobService.claimNextJob('CONTENT_GENERATION');
        if (!job) break;
        this.activeCount += 1;
        void this.processJob(job)
          .catch((error) => {
            console.error('[content-worker] unhandled job error', { jobId: job.id, err: error });
          })
          .finally(() => {
            this.activeCount -= 1;
          });
      }
    } catch (error) {
      console.error('[content-worker] poll error', { err: error });
    }
  }
}

export const contentWorker = new ContentWorker();
